...
public final class CameraManager {
  ...
  public static Point getCR(){
	...
	try {
		...
		//根据屏幕宽高获取适合的宽高像素
		Point  sr = new Point(DeviceInfo.sScreenWidth, DeviceInfo.sScreenAllHeight);
   ...
   * 设置改进的预览成像方案
   * @param sr 改进后的值
   * <br/>Create By: yanglei Email:yanglei@dcloud.io at 2014-5-29 下午03:18:26
   */
  public void reviseScreenResolution(Point sr){
	  configManager.reviseScreenResolution(sr);
  }
  /**
...
//      rect.bottom = rect.bottom * cameraResolution.y / screenResolution.y;
      ...
      rect = DetectorViewConfig.getInstance().getDetectorRect();
//    	Point screenResolution = DetectorViewConfig.getInstance().screenResolution;
    	Point screenResolution =configManager.getScreenResolution();
    	int dl = (int)((rect.top ) * cameraResolution.x / screenResolution.y);
    	int dt = (int)((screenResolution.x - rect.right) * cameraResolution.y / screenResolution.x);
    	int dw = (int)(rect.height() * cameraResolution.x / screenResolution.y);
    	int dh = (int)(rect.width() * cameraResolution.y / screenResolution.x);
    	int min_w_h = Math.max(dw, dh);
    	rect = new Rect(dl,dt,dl + min_w_h,dt + min_w_h);
