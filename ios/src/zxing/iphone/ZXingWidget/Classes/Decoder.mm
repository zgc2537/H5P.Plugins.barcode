...
extern BarcodeFormat CBarcodeFormatConvert(zxing::BarcodeFormat value);
      ...
      for (FormatReader *reader in formatReaders) {
        ...
        try {
          ...
          if (decoderResult) [decoderResult release];
            zxing::BarcodeFormat barcodeFormat = result->getBarcodeFormat();
          decoderResult = [[TwoDDecoderResult alloc] initWithText:resultString
                                                           points:points
                                                           format:CBarcodeFormatConvert(barcodeFormat)];
