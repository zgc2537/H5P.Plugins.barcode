...
- (id)initWithDelegate:(id<ZXingDelegate>)scanDelegate
    ...
    if (self) {
        ...
        beepSound = -1;
        decoding = NO;
        OverlayView *theOverLayView = [[OverlayView alloc] initWithFrame:self.bounds
                                                            oneDMode:oneDMode];
...
- (void)decoder:(Decoder *)decoder didDecodeImage:(UIImage *)image
    usingSubset:(UIImage *)subset withResult:(TwoDDecoderResult *)twoDResult {
    [self presentResultForString:[twoDResult text]];
    [self presentResultPoints:[twoDResult points] forImage:image usingSubset:subset];
    // now, in a selector, call the delegate to give this overlay time to show the points
    NSArray *args = [NSArray arrayWithObjects:
                     image,
                     [[[twoDResult text] copy] autorelease],
                     [NSNumber numberWithInt:twoDResult.barcodeFormat], nil];
    [self performSelector:@selector(notifyDelegate:) withObject:args afterDelay:0.0];
    decoder.delegate = nil;
}

- (void)notifyDelegate:(id)args {
  [delegate zxingController:self
            withDecodeImage:[args objectAtIndex:0]
              didScanResult:[args objectAtIndex:1]
              didScanFormat:[[args objectAtIndex:2] intValue]];
}
...
- (void)initCapture {
  ...
  //   self.prevLayer.transform = CATransform3DMakeScale(2.0f, 2.0f, 2.0f);
    [self orientationChange];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(orientationChange) name:UIDeviceOrientationDidChangeNotification object:nil];

    // NSLog(@"prev %p %@", self.prevLayer, self.prevLayer);
    self.prevLayer.frame = self.bounds;
   // AVCaptureConnection *avCapConnection = self.prevLayer.connection;
   // avCapConnection.videoScaleAndCropFactor = 4.5;
...
- (void) setVideoOrientation:(AVCaptureVideoOrientation)videoOrientation {
    if ( [PTDeviceOSInfo systemVersion] >= PTSystemVersion7Series ) {
        if ([[self.prevLayer connection] isVideoOrientationSupported]) {
            [[self.prevLayer connection] setVideoOrientation:videoOrientation];
        }
    } else {
        if ( [self.prevLayer isOrientationSupported] ) {
            self.prevLayer.orientation = videoOrientation;
        }
    }
}
- (void) orientationChange {
    if ( UIDeviceOrientationPortrait == [UIDevice currentDevice].orientation ) {
        // self.prevLayer.orientation = AVCaptureVideoOrientationPortrait;
       ...
       // self.prevLayer.orientation = AVCaptureVideoOrientationLandscapeRight;
        ...
        //  self.prevLayer.transform = CATransform3DMakeRotation(radians(90.0), 0.0, 0.0, 1.0);
       ...
       // self.prevLayer.orientation = AVCaptureVideoOrientationPortraitUpsideDown;
      ...
      //              UIImageWriteToSavedPhotosAlbum([UIImage imageWithCGImage:capture], nil, nil, nil);
        ...
        
        {
           ...
           // NSLog(@"%f %f", width, height);
            ...
            CGSize layerSize = self.prevLayer.bounds.size;
            float tmp1 = layerSize.width;
            if ( layerSize.width < layerSize.height ) {
                layerSize.width = layerSize.height;
                layerSize.height = tmp1;
            }
            
            cropRect.origin.x = ((layerSize.width-cropRect.size.width)/2)*width/layerSize.width;
            cropRect.origin.y = ((layerSize.height-cropRect.size.height)/2)*height/layerSize.height;
            cropRect.size.width *=width/layerSize.width;
            cropRect.size.height *=height/layerSize.height;
        }
